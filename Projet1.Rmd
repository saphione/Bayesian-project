...
```{r}
posterior_samples <- as.mcmc(partial_pooling_fit$BUGSoutput$sims.matrix)

a_samples <- partial_pooling_fit[["BUGSoutput"]][["mean"]][["a"]]
b_bare_samples <- partial_pooling_fit[["BUGSoutput"]][["mean"]][["b.bare"]]
b_bare_carre_samples <- partial_pooling_fit[["BUGSoutput"]][["mean"]][["b.bare_carre"]]
b_height_samples <- partial_pooling_fit[["BUGSoutput"]][["mean"]][["b.height"]]
b_height_carre_samples <- partial_pooling_fit[["BUGSoutput"]][["mean"]][["b.height_carre"]]
...
bare_values_scale <- scale(hoopoe$bare)
height <- hoopoe$heightC
height_scale <- scale(hoopoe$heightC)

logit_p <- a_mean + b_bare_mean*bare_values_scale + b_bare_carre_mean*(bare_values_scale)^2 + b_height_mean*height_scale + b_height_mean_carre*(height_scale)^2
p_mean <- plogis(logit_p)
...
ggplot(df, aes(x = bare_values, y = p_mean)) +
...
ggplot(df, aes(x = height, y = p_mean)) +
...
```{r}
df2 <- matrix(nrow=10, ncol=4)
sequence_bare <- seq(0,100, by= 9)
sequence_height <- seq(5,40, by= 4)
df2 <- as.matrix(df2)
colnames(df2) <- c("proba_bare", "proba_height", "bare", "height")

for (i in 1:(length(sequence_bare)-1)){
  if (i == 1){
    df2[i,1] <- mean(df[df[,1]<= sequence_bare[i],3])
    df2[i,2] <- mean(df[df[,2]<= sequence_height[i],3])
  }
  else {
    df2[i,1] <- mean(df[df[,1] > sequence_bare[i] & df[,1] <= sequence_bare[i+1],3])
    df2[i,2] <- mean(df[df[,2] > sequence_height[i] & df[,2] <= sequence_height[i+1],3])
  }
  df2[i,3] <- sequence_bare[i]
  df2[i,4] <- sequence_height[i]
}

ggplot(df2, aes(x = bare, y = proba_bare)) +
  labs(
    title = "Probabilité en fonction de % Bareground",
    x = "% Bareground",
    y = "Probabilité"
  ) +
  theme_minimal() +
...
ggplot(df2, aes(x = height, y = proba_height)) +
  labs(
    title = "Probabilité en fonction de % Vegetation Height",
    x = "% Vegetation Height",
    y = "Probabilité"
  ) +
  theme_minimal() +
